...
Facade.prototype.enabled = function (integration) {
  ...
  if (options.hasOwnProperty(integration)) {
    enabled = this.explicitEnabled(integration);
  }
...
/**
 * Checks whether the user explicitly enabled the integration. Some integrations
 * require an explicit check.
 */

Facade.prototype.explicitEnabled = function (integration) {
  var options  = this.options()
    , explicit = false;

  if (options.hasOwnProperty(integration)) {
    var settings = options[integration];
    if (typeof settings === 'boolean') explicit = settings;
    else explicit = true;
  }
